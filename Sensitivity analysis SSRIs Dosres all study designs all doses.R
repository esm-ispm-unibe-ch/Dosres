...
##################################################################
#     SENSITIVITY ANALYSIS using only SSRIs
#     ANALYSIS INCLUDING all dose and all drugs
#################################################################

pdf("Sensitivity analysis SSRIs Meta-analytic dose plots.pdf")
sink("Sensitivity analysis SSRIs Meta-analytic dose-response analysis.txt")
...
DOSESSRIs=cleandosresdata.fun(DOSESSRIs,Study_No,logRR,Responders,No_randomised,hayasaka_ddd,"exc")
DOSESSRIs=cleandosresdata.fun(DOSESSRIs,Study_No,logRRdrop,Dropouts_total,No_randomised,hayasaka_ddd,"excdrop")
DOSESSRIs=cleandosresdata.fun(DOSESSRIs,Study_No,logRRdropAE,Dropouts_sideeffects,No_randomised,hayasaka_ddd,"excdropAE")

##REPORTING
cat("\n", paste("There are", length(unique(DOSESSRIs$Study_No)), "studies comparing all doses .", "\n"))
cat("which include the drugs:", unique(DOSESSRIs$Drug), "\n")
...
mymoredata=DOSESSRIs[DOSESSRIs$exc==F,]

cat("\n-------- Linear response -----------------------------\n")
cat(paste("******For the linear model there are", length(unique(mymoredata$Study_No)), "studies", "\n"))
text=paste(length(unique(mymoredata$Study_No)),"studies comparing SSRIs  for response")
...
mymoredata=DOSESSRIs[DOSESSRIs$excdrop==F,] 


cat("\n-------- Linear dropout -----------------------------\n")
cat(paste("\n******For the linear model there are", length(unique(mymoredata$Study_No)), "studies", "\n"))
text=paste(length(unique(mymoredata$Study_No)),"studies comparing SSRIs for dropout")
...
cat("\n-----------------------------------------------\n")
cat("\n-------- DROPOUT DUE TO AE --------------------\n")
mymoredata=DOSESSRIs[DOSESSRIs$excdropAE==F,] 

cat("\n-------- Linear dropout AE -----------------------------\n")
cat(paste("**** For the linear model", length(unique(mymoredata$Study_No)), "studies", "\n"))
text=paste(length(unique(mymoredata$Study_No)),"studies comparing SSRIs for dropout due to AE")
